...
class AccuracyComparator:
    ...
    def compare(self, node1: Node, node2: Node) -> float:
        ...
        """
        try:
            system_prompt, user_prompt = self._build_comparison_prompt(self.input_data, node1.parsed_yaml, node1.sample_result, node2.parsed_yaml, node2.sample_result)
            messages = [
            ...
            for attempt in range(self.max_retries):
                try:
                    ...
                    if attempt == self.max_retries - 1:
                        print(f"LLM comparison failed after {self.max_retries} attempts: {e}")
                        return 0.0  # Default to equal if all attempts fail
                    continue
            return 0.0
            
        except Exception as e:
            print(f"Error comparing plans {node1.yaml_file_path} vs {node2.yaml_file_path}: {e}")
            return 0.0
