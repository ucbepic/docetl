...
class OpContainer:
    ...
    def __init__(self, name: str, runner: "DSLRunner", config: Dict, **kwargs):
        ...
        if not self.selectivity:
            ...
            if self.config.get("type") in ["map", "parallel_map", "code_map", "resolve", "gather"]:
                self.selectivity = 1
    ...
    def optimize(self):
        ...
        sample_size_needed = self.runner.optimizer.sample_size_map.get(self.config["type"])
        # run the children to get the input data for optimizing this operation
        input_data = []
        for child in self.children:
            input_data.append(child.next(is_build=True, sample_size_needed=sample_size_needed)[0])
        ...
        if self.config.get("optimize", False):
            if self.config["type"] not in SUPPORTED_OPS:
                self.runner.console.print(f"[red]Operation {self.name} is not supported for optimization. Proceeding without optimizing it.[/red]")
            else:
                ...
                with self.runner.console.status(
                    ...
                    for retry in range(
                        ...
                        try:
                            if self.config.get("type") in ["map", "filter"]:
                                ...
                                optimized_ops = [op_config] if not optimized_config.get("empty", False) else []
                                self.runner.total_cost += cost
                                ...
                                # Replace old op map
                                self.runner.op_container_map = {k: v for k, v in self.runner.op_container_map.items() if k not in [self.children[0].name, self.children[1].name]}
                                ...
                                self.runner.op_container_map[f"{curr_step_name}/scan_{new_left_name}"] = self.children[0]
                                self.runner.op_container_map[f"{curr_step_name}/scan_{new_right_name}"] = self.children[1]
                                ...
                                for step_name, step_obj, operations in reversed(new_steps):
                                    ...
                                    self.runner.op_container_map[f"{step_name}/boundary"] = step_boundary_container
                                    # Point the equijoin op container to this step boundary
                                    local_last_op_container.add_child(step_boundary_container)
                                    ...
                                    for op in operations:
                                        ...
                                        self.runner.op_container_map[f"{step_name}/{op['name']}"] = op_container
                                    ...
                                    self.runner.op_container_map[f"{step_name}/scan_{step_obj['input']}"] = scan_op_container
                    ...
                    if len(optimized_ops) > 0:
                        ...
                        for idx, op in enumerate(list(reversed(optimized_ops))):
                            ...
                            self.runner.op_container_map[f"{curr_step_name}/{op['name']}"] = op_container
                            local_last_op_container.add_child(op_container)
                            local_last_op_container = op_container
        ...
        # Figure out the sample size needed for this operation from the sample size map
        # It may be None if the operation is not in the sample size map, which means we will get all the data
        sample_size_needed = self.runner.optimizer.sample_size_map.get(new_head_pointer.config["type"])
    ...
    def next(self, is_build: bool = False, sample_size_needed: int = None) -> Tuple[List[Dict], float, str]:
        ...
        if self.selectivity and sample_size_needed:
            input_sample_size_needed = int(math.ceil(sample_size_needed / self.selectivity))
        else:
        ...
        if self.is_equijoin:
            ...
            left_data, left_cost, left_logs = self.children[0].next(is_build, input_sample_size_needed)
            right_data, right_cost, right_logs = self.children[1].next(is_build, input_sample_size_needed)
        ...
        # Handle standard operations with single input
            ...
            input_data, input_cost, input_logs = self.children[0].next(is_build, input_sample_size_needed)
            cost += input_cost
...
class StepBoundary(OpContainer):
    def next(self, is_build: bool = False, sample_size_needed: int = None) -> Tuple[List[Dict], float, str]:
        ...
        output_data, step_cost, step_logs = self.children[0].next(is_build, sample_size_needed)

        # Print step logs
        self.runner.datasets[self.name.split("/")[0]] = Dataset(self, "memory", output_data)
