...
class ChangeModelDirective(Directive):
    ...
    nl_description: str = Field(default="Rewrites an operator to use a different LLM model, changing the underlying engine while keeping all logic and prompts the same. Can be applied to multiple operators at once.")
    ...
    def instantiate(self, global_default_model, operators: List[Dict], target_ops: List[str], agent_llm: str, message_history: list = [], optimize_goal = "acc", temperature = 0.8) -> tuple:
        ...
        if inst_error == len(target_ops): return None, message_history
