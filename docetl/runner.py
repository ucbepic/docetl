...
class DSLRunner(ConfigWrapper):
    ...
    def load_output_by_step_and_op(
        ...
        if not self.checkpoint_manager:
            return None
        return self.checkpoint_manager.load_output_by_step_and_op(
            step_name, operation_name
        )

    def load_output_as_dataframe(self, step_name: str, operation_name: str):
        """
        ...
        if not self.checkpoint_manager:
            return None
        return self.checkpoint_manager.load_output_as_dataframe(
            step_name, operation_name
        )
    ...
    def list_outputs(self) -> List[Tuple[str, str]]:
        ...
        if not self.checkpoint_manager:
            return []
        return self.checkpoint_manager.list_outputs()
    ...
    def get_checkpoint_size(self, step_name: str, operation_name: str) -> Optional[int]:
        ...
        if not self.checkpoint_manager:
            return None
        return self.checkpoint_manager.get_checkpoint_size(step_name, operation_name)
    ...
    def get_total_checkpoint_size(self) -> int:
        ...
        if not self.checkpoint_manager:
            return 0
        return self.checkpoint_manager.get_total_checkpoint_size()
