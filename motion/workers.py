...
def chunk_data(
    data: List[Tuple[str, Any, Any]], chunk_size: int
    ...
    if isinstance(operation.operator, FlatMapper):
        ...
        with multiprocessing.Pool(num_workers) as pool:
            results = pool.starmap(
                flatmap_worker, [(chunk, operation) for chunk in chunks]
            )
        ...
        with multiprocessing.Pool(num_workers) as pool:
            results = pool.starmap(map_worker, [(chunk, operation) for chunk in chunks])
        ...
        with multiprocessing.Pool(num_workers) as pool:
            results = pool.starmap(
                filter_worker, [(chunk, operation) for chunk in chunks]
            )
        ...
        processed_data, errors = reduce_worker(data, operation)
