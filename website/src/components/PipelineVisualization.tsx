...
import { Sparkles, Cog, ArrowRight, ArrowDown, Play, ChevronRight, ChevronLeft, Zap, Pencil } from 'lucide-react';
...
const colOrder = ["date", "title", "content", "themes", "theme", "viewpoints", "report"]
...
[
  ...
  { 
    ...
    description: "This step flattens the nested theme structure. The input to this step consists of one document per debate transcript, but the output of this step will consist of a document per theme identified."
  },
  { 
    synthesized: true,
    ...
    color: "yellow",
    isLLM: true,
    exampleVars: {"input1.theme": "Medicare and Social Security", "input1.viewpoints": `Vice President Biden: The cuts proposed would harm millions; we have extended Medicare's
life by making smart cuts, not vouchers. Congressman Ryan: Medicare and Social Security are going bankrupt; we must
reform them to protect future generations.`, "input2.theme": "Social Security and Medicare", "input2.viewpoints": `Dole speaks to the need for reform and preserving benefits, stating, "We want to make 
...
programs, mentioning, "Social Security is stable until... at least the third decade of the next century."`},
    exampleOutput: [{"equal": "True", "(resolved) theme": "Social Security and Medicare"}],
...
const PipelineVisualization = () => {
  ...
  const handleStepClick = useCallback(async (index: number) => {
    ...
    setOutputs([]);
    setCurrentPage(1);
    await streamContent(pipelineSteps[index].prompt || pipelineSteps[index].description || '');
  ...
  const renderTableCell = (value: any) => {
    ...
    const truncatedValue = stringValue.substring(0, 100) + (stringValue.length > 100 ? '...' : '');
    ...
    return (
      <TooltipProvider>
        <Tooltip>
          ...
          <TooltipContent>
            <p className="max-w-md overflow-auto max-h-60 whitespace-pre-wrap">{stringValue}</p>
          </TooltipContent>
        </Tooltip>
      </TooltipProvider>
    );
  ...
  const paginatedOutputs = outputs.slice((currentPage - 1) * rowsPerPage, currentPage * rowsPerPage);
  ...
  const renderPromptWithExampleVars = (prompt: string, exampleVars: { [key: string]: any }) => {
    return (
      <span>
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="text-left">
          <h2 className="text-2xl font-bold mb-2">US Presidential Debate Analysis</h2>
      ...
      <p className="text-sm text-muted-foreground text-left">
        This pipeline analyzes themes in US presidential debates dating back to 1960,
        summarizing the evolution of the viewpoints of Democrats and Republicans for each theme. It cost us $0.29 to run (and $0.86 to optimize).
      </p>
      <p className="text-sm text-muted-foreground text-left">
      ...
      <div className="flex flex-col md:flex-row justify-center items-center gap-2 mb-8 overflow-x-auto py-4">
          ...
          <React.Fragment key={step.name}>
            <motion.div
              className={`p-2 rounded-lg shadow-sm cursor-pointer flex-shrink-0 w-full md:w-44
                ...
                ${!step.isLLM ? 'scale-95' : ''}`}
              animate={
                isRunning && index === currentStep ? { scale: [1, 1.05, 1] } : {}
              }
            ...
            >
              <div className="flex flex-row md:flex-col items-center justify-start md:justify-center text-left md:text-center h-full">
                <div className="mr-2 md:mr-0 md:mb-1">
                  {step.isLLM ? (
                    <Sparkles className={`w-4 h-4 ${completedSteps.includes(index) || index === currentStep ? 'text-primary' : 'text-secondary'}`} />
                  ) : (
                    <Cog className={`w-4 h-4 ${completedSteps.includes(index) || index === currentStep ? 'text-primary' : 'text-secondary'}`} />
                  )}
                </div>
                <div>
                  ...
                  {step.isLLM && step.outputDescription && (
                    <p className="text-xs italic text-muted-foreground mt-1 line-clamp-2">{step.outputDescription}</p>
                  )}
                </div>
              </div>
            </motion.div>
            {index < pipelineSteps.length - 1 && (
              <>
                <ArrowRight size={16} className="text-muted-foreground hidden md:block flex-shrink-0" />
                <ArrowDown size={16} className="text-muted-foreground md:hidden flex-shrink-0" />
              </>
            )}
          </React.Fragment>
        ))}
        ...
        {currentStep !== -1 && (
          <div className="border-t border-border pt-6 text-left">
            <h3 className="text-xl font-semibold mb-2">{pipelineSteps[currentStep].name}</h3>
            <p className="text-sm text-muted-foreground mb-4">
              Operator type: <a href={opTypeToDocLink[pipelineSteps[currentStep].type as keyof typeof opTypeToDocLink]} 
                                target="_blank" rel="noopener noreferrer" 
                                className="text-primary hover:underline">
                {pipelineSteps[currentStep].type}
              </a>
              {pipelineSteps[currentStep].synthesized ? " (Synthesized by DocETL's optimizer" : " (User-defined"} {renderStepIcon(pipelineSteps[currentStep])})
            </p>
            
            <div className="mb-6">
            <h4 className="text-sm font-semibold mb-2">
              {pipelineSteps[currentStep].isLLM ? "LLM Prompt:" : "Description:"}
              {pipelineSteps[currentStep].exampleOutput && renderExampleOutput(pipelineSteps[currentStep].exampleOutput)}
            </h4>
            <pre className="text-sm whitespace-pre-wrap overflow-x-auto bg-muted p-4 rounded-md">
              {pipelineSteps[currentStep].exampleVars
                ? renderPromptWithExampleVars(streamedContent, pipelineSteps[currentStep].exampleVars)
                : streamedContent}
            </pre>
          </div>
                ...
                <div className="flex items-center space-x-2">
                  ...
                  {showNextButton && (
                    <Button onClick={handleNextStep} variant="outline" size="sm">
                      {currentStep === pipelineSteps.length - 1 ? 'Finish' : 'Next Step'}<ChevronRight className="ml-1 h-4 w-4" />
                    </Button>
                  )}
                </div>
                <div className="flex items-center space-x-2">
                  ...
                  />
                  <label htmlFor="show-outputs" className="text-sm">Show All Step Outputs</label>
                </div>
              </div>
            )}
            ...
            {showOutputs && !isStreaming && (
              <div>
                {isLoadingOutputs ? (
                  <p className="text-center text-muted-foreground">Loading outputs...</p>
                ) : outputs.length > 0 ? (
                  <>
                    <Table>
                      <TableHeader>
                        <TableRow>
                          {Object.keys(outputs[0]).filter(key => !ignoreCols.includes(key)).map((key) => (
                            <TableHead key={key}>
                              {pipelineSteps[currentStep].outputCols.includes(key) ? <strong>{key}</strong> : key}
                            </TableHead>
                          ))}
                        </TableRow>
                      </TableHeader>
                            ...
                            {Object.entries(row)
                              ...
                              .map(([key, value], cellIndex) => (
                                <TableCell key={cellIndex}>
                                  {pipelineSteps[currentStep].outputCols.includes(key) ? (
